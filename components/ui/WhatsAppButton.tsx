...
// le ofrece al visitante dejar su número, que queda guardado como contacto
// en Nuku OS — tabla `guests`, source "whatsapp_widget" — para poder hacer
// seguimiento aunque no termine reservando; "Escribir directo" abre
// WhatsApp igual, sin guardar nada). Se adapta acá sin la capa de
// idioma (i18n) que tenía esa versión, porque este sitio no es bilingüe.
//
// No se muestra mientras site.whatsapp siga sin confirmar (Sofía todavía
// está definiendo qué número usar) — no inventamos un número.
...
export default function WhatsAppButton() {
  ...
  if (site.whatsapp === TODO_PLACEHOLDER) return null;

  function openChat() {
    const number = whatsappDigits(site.whatsapp);
    const text = encodeURIComponent(whatsappWidget.chatMessage);
  ...
  async function handleContinue() {
    ...
    try {
      await fetch(`${NUKU_OS_ORIGIN}/api/public/leads`, {
        ...
        body: JSON.stringify({ account_id: reserva.nukuOsAccountId, phone: trimmed, source: "whatsapp_widget" }),
      });
    } catch {
    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3 sm:bottom-7 sm:right-7">
      {open && (
        <div className="w-72 rounded-2xl border border-stone/10 bg-warm-white p-4 shadow-[0_12px_36px_rgba(0,0,0,0.18)]">
          <p className="text-sm font-medium text-stone">{whatsappWidget.heading}</p>
          <p className="mt-1 text-xs leading-relaxed text-stone-soft">{whatsappWidget.subtext}</p>
          <input
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
            placeholder={whatsappWidget.phonePlaceholder}
            type="tel"
          ...
          >
            {sending ? whatsappWidget.sending : whatsappWidget.continueBtn}
          </button>
          <button
            type="button"
            onClick={openChat}
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-label={whatsappWidget.openAria}
